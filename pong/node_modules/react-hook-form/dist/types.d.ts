...
export declare type ManualFieldError<FormValues extends FieldValues> = {
    name: IsFlatObject<FormValues> extends true ? Extract<keyof FormValues, string> : string;
    type: string;
    types?: MultipleFieldErrors;
    message?: Message;
};
...
export declare type NestDataObject<FormValues, Value> = {
    [Key in keyof FormValues]?: FormValues[Key] extends Array<infer U> ? IsAny<U> extends true ? any : unknown extends U ? Value[] : object extends U ? Value[] : U extends Date ? Value[] : FormValues[Key] extends FileList ? Value[] : U extends object ? NestDataObject<U, Value>[] : Value[] : IsAny<FormValues[Key]> extends true ? any : unknown extends FormValues[Key] ? Value : object extends FormValues[Key] ? Value : FormValues[Key] extends Date ? Value : FormValues[Key] extends FileList ? Value : FormValues[Key] extends object ? NestDataObject<FormValues[Key], Value> : Value;
};
...
export declare type CustomElement<FormValues extends FieldValues> = {
    name: IsFlatObject<FormValues> extends true ? Extract<keyof FormValues, string> : string;
    type?: string;
    value?: any;
    checked?: boolean;
...
export declare type FieldElement<FormValues extends FieldValues = FieldValues> = HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | CustomElement<FormValues>;
